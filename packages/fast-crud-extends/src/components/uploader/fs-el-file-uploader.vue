...
export default {
  ...
  computed: {
    ...
    avatarUrl() {
      if (this.fileList.length > 0) {
        ...
        logger.debug("file,", file, file.status);
  ...
  methods: {
    ...
    handleUploadProgress(event, file) {
      if (this._elProps.listType === "avatar") {
        logger.debug("progress", event, file);
    ...
    handleUploadFileSuccess(res, file, fileList) {
      ...
      for (const item of fileList) {
        ...
        //   logger.debug('当前文件上传完成，等待剩下的文件全部上传成功后再更新value')
        //   return
        // }
      ...
      logger.debug("handleUploadFileSuccess list", list, res);
      this.emit(res, list);
    },
    isHasUploadingItem() {
    ...
    handleUploadFileError(err, file, fileList) {
      logger.error("文件上传失败", err, file, fileList);
    ...
    doUpload(option) {
      ...
      if (!_.isEmpty(this._elProps.action)) {
        config.action = this._elProps.action;
      }
      if (!_.isEmpty(this._elProps.name)) {
        config.name = this._elProps.name;
      }
      if (!_.isEmpty(this._elProps.data)) {
        config.data = this._elProps.data;
      }
      if (!_.isEmpty(this._elProps.headers)) {
        config.headers = this._elProps.headers;
      }
      if (!_.isEmpty(this.custom)) {
