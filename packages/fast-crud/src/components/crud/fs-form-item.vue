...
import { computeImmediate, useMerge, useUi } from "../../use";
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const computedHelperPosition = computed(() => {
      return cprops.value.item?.helper?.position || (cprops.value.helper as any)?.position;
    });
    const computedHelperTooltip = computed(() => {
      return merge({}, cprops.value.item.helper?.tooltip, (cprops.value.helper as any)?.tooltip);
    });
    const computedKey = computed(() => {
      if (cprops.value.item == null) {
        return;
      }
      if (cprops.value.item.key.indexOf(".") >= 0) {
        return cprops.value.item.key.split(".");
      }
      return cprops.value.item.key;
    });
