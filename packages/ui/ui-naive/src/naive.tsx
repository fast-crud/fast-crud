...
export class Naive implements UiInterface {
  ...
  notification: NotificationCI = creator<NotificationCI>({
    ...
    getInstance() {
      if (!this.instance) {
        throw new Error(
          "您还未设置ui,先安装依赖@fast-crud/ui-interface,然后在use(FastCrud)前安装ui，app.use(UiXxx)，还需要用fs-ui-context包裹router-view，请参考http://fast-crud.docmirror.cn/guide/start/integration.html#_5-naiveui%E7%9A%84%E9%A2%9D%E5%A4%96%E6%93%8D%E4%BD%9C"
        );
      }
      if (this.instance instanceof Function) {
        return this.instance();
